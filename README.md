# Вэб-кошелек
RESTful API приложение на фреймворке Django Rest Framework: 
обеспечивает просмотр баланса, перевод и снятие денежных средств

## Основные задачи
1. Разработать приложение, которое по REST принимает запрос вида:
POST api/v1/wallets/<WALLET_UUID>/operation
2. Реализовать операции снятия денежных средств/перевод денежных средств, а также просмотра баланса кошелька
...
4. `.env.sample` - текстовый файл с настройками переменных окружений для `.env`
...
8. `mypy.init` - текстовый файл для настроек линтера mypy
9. `Dockerfile` - текстовый файл, который содержит инструкции для автоматической сборки Docker-образа
10. `docker-compose.yml` - файл в формате YAML, который описывает и настраивает многоконтейнерное приложение Docker
11. `README.md` - текстовый файл с описанием проекта
...
2. Подключение к ВМ:
`ssh -l test_task 51.250.109.134`
...
`ssh -l test_task 51.250.109.134`, где
